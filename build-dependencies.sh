...
#
# Builds the pinned native dependencies for tidesdb-java-unified:
#
#   1. cleans the previous native outputs in ./build;
#   2. clones the pinned upstream sources (TidesDB, tidesdb-java, zstd, LZ4, Snappy);
#   3. builds the compression libraries as static archives;
#   4. builds TidesDB as a shared library (libtidesdb.so) with statically-linked
#      compression, and the JNI bridge (libtidesdb_jni.so);
#   5. installs the pinned tidesdb-java artifact into the local Maven repository;
#   6. copies the .so files into build/generated-resources.
#
# All clone/build work happens under the temporary ./workspace directory, which
# is emptied on both success and failure.  ./build is created and cleaned before
# ./workspace so stale generated resources never survive a run.
...
# --- Source, build, and install directories under ./workspace ---
...
# ./workspace is temporary work space: it must exist and be empty when this
# script finishes, no matter how the build ends.
empty_workspace() {
...
# ./build holds the generated native resources: it must exist and be clean
# BEFORE ./workspace is handled, so stale artifacts never survive a run.
empty_build() {
...
# Remove previous outputs so a stale libtidesdb*.so never survives a run.
empty_build

# Remove any leftover workspace from an interrupted earlier run.
empty_workspace
...
preflight() {
    ...
    if [[ ! "$java_ver" =~ ^[0-9]+$ ]] || [ "$java_ver" -lt 11 ]; then
        echo "[PREFLIGHT] FAIL: Java >= 11 required, detected $(java -version 2>&1 | head -1)"
        ((errors++))
    fi
    ...
    [ ! -f "$PROJECT_DIR/dependencies.properties" ] && { echo "[PREFLIGHT] FAIL: dependencies.properties not found"; ((errors++)); }
...
build_jni() {
    ...
    # Skip RPATH so the .so does not embed a workspace build path.
...
# ===================================================================
# Copy native libraries to generated-resources
# ===================================================================
copy_libs() {
...
phase "Copy native libraries to generated-resources"
copy_libs

# --- Success ---
echo ""
echo "===== DEPENDENCIES BUILD SUCCESS ====="
